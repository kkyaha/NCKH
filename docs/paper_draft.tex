...
\begin{tikzpicture}[
    ...
    \node[good, below=of l3] (out) {Verified $do(X{=}x)$, confidence $\in\{$high, medium, low, refused$\}$};
...
None of this is uniform across metrics. Memory forecasts at $2$--$4\%$ MAPE on both testbeds throughout this paper (Table~\ref{tab:rq1}), with none of the three CPU failure modes above. The ceiling is a property of CPU specifically -- driven by garbage collection, OS scheduling, and background health-check work no request-count metric observes -- not a property of the pipeline. This is also consistent with why prior SCM-based microservice work stays retrospective (Section~\ref{sec:background}): attributing an already-observed deviation to its ancestors is a relative comparison a noisy target still supports; forecasting its absolute future value is not.

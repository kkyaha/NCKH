...
## `feasibility/` — Nhánh khả thi (RQ5) — đóng băng trước, đánh giá sau
***THỨ TỰ QUAN TRỌNG**: `freeze_*` phải chạy TRƯỚC khi đo ramp, nếu không thì không còn là dự đoán tiền đăng ký.*
...
## `model_eval/` — So sánh mô hình & thống kê (RQ3 Measurability)
*Sinh `05_model_comparison.csv`, các kiểm định có hiệu chỉnh Holm, và hình cho bài báo.*
...
## `parser/` — Parser & Scope Gate (RQ1 Necessity, RQ2 Guarantee)
*Benchmark parser 50 prompt, các lớp guard, hiệu chỉnh conformal.*
...
## `legacy/` — Hệ đánh số CŨ (RQ6–RQ12) — giữ để tái lập
*⚠ Số RQ trong tên các file này thuộc khung **CŨ**, KHÔNG phải khung 5 RQ hiện tại. Xem bảng dịch ở `README.md` gốc. Giữ lại vì chúng là nguồn gốc của các kết quả âm đã báo cáo.*
